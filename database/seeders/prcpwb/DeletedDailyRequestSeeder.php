...
class DeletedDailyRequestSeeder extends Seeder
{
    /**
    ...
    {
        DB::connection('prcpwb')->statement('TRUNCATE TABLE "PRCPWB_TRHDELETEDDAILYREQUESTS" RESTART IDENTITY CASCADE');
        $oldDatas = DB::connection('mysql_legacy')->table('sim_proc_poweb_deleted_dr')->get();

        foreach ($oldDatas as $old) {
            DB::connection('prcpwb')->table('PRCPWB_TRHDELETEDDAILYREQUESTS')->insert([
        ...
        $maxId = DB::connection('prcpwb')->table('PRCPWB_TRHDELETEDDAILYREQUESTS')->max('IID');
        if ($maxId) {
            DB::connection('prcpwb')->statement("SELECT setval('\"SQ_TRHDELETEDDAILYREQUESTS_IID\"', $maxId)");
        }
    }
}
